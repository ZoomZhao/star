...
产物为 `artifacts/star-explorer-native.apk`（0.2.13-native，调试签名），默认访问 `https://star.zoomzhao.com`，不依赖 USB 转发。Android 已移除 BridgeActivity 和 WebView，界面、表单、图片绘制、触感、星星动画、文件选择和网络请求均为原生实现。不要再运行 Capacitor sync。
...
- 家长审批和代完成通过点亮星星、点选完成次数来分配奖励；超过 5 星时用加减按钮精确调整，无需操作数字输入框。可分配每次星星 1–101、本次完成次数 1–20（不能超过每日剩余额度），实发星星为两者乘积。孩子仍按一次提交，待确认次数同样占用额度。旧版加星、少发请求继续兼容。
